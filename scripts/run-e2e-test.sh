...
function start_all_servers() {(
  ...
  build_app_and_serve() {(
    ...
    #ckck#    pub install
    #ckck#    pub build
    #ckck#    rsync -rl --exclude packages web/ build/web/
    #ckck#    rm -rf build/web/packages
    #ckck#    ln -s $PWD/packages build/web/packages
    ...
    (
      set -vx
      cd build/web && python -m SimpleHTTPServer ${!PORT_VAR}
    ) &
  )}
